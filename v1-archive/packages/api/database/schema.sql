...
CREATE TABLE IF NOT EXISTS transactions (
  ...
  protection_warnings JSONB,
  license_validation JSONB,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- Revoked tokens table for blacklist management
...
DROP POLICY IF EXISTS "Users manage own sessions" ON auth_sessions;
CREATE POLICY "Users manage own sessions" ON auth_sessions
  USING (auth.uid()::uuid = user_id)
  WITH CHECK (auth.uid()::uuid = user_id);

DROP POLICY IF EXISTS "Service can view revoked tokens" ON revoked_tokens;
CREATE POLICY "Service can view revoked tokens" ON revoked_tokens
  FOR SELECT USING (auth.role() = 'service_role');

DROP POLICY IF EXISTS "Service can insert revoked tokens" ON revoked_tokens;
CREATE POLICY "Service can insert revoked tokens" ON revoked_tokens
  FOR INSERT WITH CHECK (auth.role() = 'service_role');

DROP POLICY IF EXISTS "Service can delete revoked tokens" ON revoked_tokens;
CREATE POLICY "Service can delete revoked tokens" ON revoked_tokens
...
DROP POLICY IF EXISTS "Publishers can view own profile" ON publishers;
...
DROP POLICY IF EXISTS "Crawlers can view own profile" ON crawlers;
...
DROP POLICY IF EXISTS "Crawlers can view own payments" ON payments;
