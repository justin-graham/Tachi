...
import { TransactionStatusCard } from '../transactions/TransactionStatusCard'
...
export function CrawlRequestForm({ onSuccess }: CrawlRequestFormProps) {
  ...
  const { data: allowance } = useUSDCAllowance(address, addresses.paymentProcessor, chainId)
  const { approve, isPending: isApproving } = useApproveUSDC(chainId)
  const { requestCrawl, isPending: isRequesting } = useRequestCrawl(chainId)
  ...
  const onSubmit = async (data: CrawlRequestForm) => {
    ...
    setTransactionHashes({}) // Reset transaction hashes
    ...
    try {
      ...
      toast.loading('Requesting USDC approval...')
      const approveHash = await approve(data.amount)
      ...
      setTransactionHashes(prev => ({ ...prev, approve: approveHash }))
      toast.loading('Waiting for approval confirmation...')

      // Then request the crawl
      toast.loading('Submitting crawl request...')
      const crawlHash = await requestCrawl(data.tokenId, data.targetUrl, data.amount)
      ...
      setTransactionHashes(prev => ({ ...prev, crawlRequest: crawlHash }))
      toast.success('Crawl request submitted! Monitor transaction status below.')
  ...
  const handleTransactionSuccess = () => {
    toast.success('All transactions confirmed successfully!')
    reset()
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      {(transactionHashes.approve || transactionHashes.crawlRequest) && (
        <div className="bg-white rounded-lg shadow-md p-6">
          <h3 className="text-lg font-semibold mb-4">Transaction Status</h3>
          <div className="space-y-4">
            {transactionHashes.approve && (
              <TransactionStatusCard
                hash={transactionHashes.approve}
                title="USDC Approval"
                description="Approving USDC spending for crawl payment"
                onSuccess={() => console.log('Approval confirmed')}
                onError={handleTransactionError}
              />
            )}
            {transactionHashes.crawlRequest && (
              <TransactionStatusCard
                hash={transactionHashes.crawlRequest}
                title="Crawl Request"
                description="Submitting crawl request to payment processor"
