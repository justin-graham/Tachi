...
export default function SignUpPage() {
  ...
  const [isLoading, setIsLoading] = useState<OAuthProvider | null>(null); // Track which button is loading
  ...
  const handleOAuthSignup = async (provider: OAuthProvider) => {
    ...
    setFeedback({
      variant: 'info',
      message: `Contacting ${providerLabels[provider]} to start signup...`,
    });

    try {
      // Store user type in session storage for after OAuth redirect
      sessionStorage.setItem('tachi-signup-user-type', userType);

      const response = await fetch(`/api/auth/${provider}?type=signup`);
      ...
      if (!response.ok) {
        setFeedback({
          variant: 'error',
          message:
            payload?.message ?? `We couldn't start ${providerLabels[provider]} signup right now. Please try again shortly.`,
        });
        setIsLoading(null);
        return;
      }
        ...
        <EnhancedCard variant="elevated" className="bg-white shadow-xl">
          <EnhancedCardHeader className="text-center pb-8 pt-10">
            ...
            <p className="text-lg leading-relaxed text-[#52796F] max-w-md mx-auto">
              Create your account to start monetizing content or accessing premium data
            </p>
          </EnhancedCardHeader>
          ...
          <EnhancedCardContent className="space-y-6">
            ...
            <div className="bg-blue-50 rounded-lg p-4">
              ...
              <p className="text-sm text-[#52796F]">
                We use OAuth to keep your account secure. We never store your passwords and you can revoke access at any time.
              </p>
            </div>
          </EnhancedCardContent>
        </EnhancedCard>
