...
async function verifyMultisigOwnership() {
  ...
  const multisigCode = await ethers.provider.getCode(VERIFICATION_CONFIG.MULTISIG_ADDRESS);
  if (multisigCode === "0x") {
  ...
  try {
    ...
    for (const expectedOwner of VERIFICATION_CONFIG.EXPECTED_OWNERS) {
      if (expectedOwner === "0x0000000000000000000000000000000000000001") continue; // Skip placeholder
      
      const isOwner = owners.some((owner: string) => owner.toLowerCase() === expectedOwner.toLowerCase());
      if (isOwner) {
  ...
  if (VERIFICATION_CONFIG.CONTRACTS.crawlNFT) {
    try {
      const crawlNFT = await ethers.getContractAt(
        "CrawlNFTUpgradeable",
        VERIFICATION_CONFIG.CONTRACTS.crawlNFT
      );
  ...
  if (VERIFICATION_CONFIG.CONTRACTS.paymentProcessor) {
    try {
      const paymentProcessor = await ethers.getContractAt(
        ...
        VERIFICATION_CONFIG.CONTRACTS.paymentProcessor
      );
...
export { verifyMultisigOwnership, VERIFICATION_CONFIG };
