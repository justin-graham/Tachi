...
exports.EMERGENCY_CONFIG = void 0;
exports.emergencyOwnershipRecovery = emergencyOwnershipRecovery;
exports.generateEmergencyMultisigTransaction = generateEmergencyMultisigTransaction;
...
exports.EMERGENCY_CONFIG = EMERGENCY_CONFIG;
/**
...
async function emergencyOwnershipRecovery() {
    ...
    if (EMERGENCY_CONFIG.CONTRACTS.crawlNFT) {
        try {
            const crawlNFT = await hardhat_1.ethers.getContractAt("CrawlNFTUpgradeable", EMERGENCY_CONFIG.CONTRACTS.crawlNFT);
    ...
    if (EMERGENCY_CONFIG.CONTRACTS.paymentProcessor) {
        try {
            const paymentProcessor = await hardhat_1.ethers.getContractAt("PaymentProcessorUpgradeable", EMERGENCY_CONFIG.CONTRACTS.paymentProcessor);
...
async function generateEmergencyMultisigTransaction() {
    ...
    for (const [contractName, contractAddress] of Object.entries(EMERGENCY_CONFIG.CONTRACTS)) {
        if (contractAddress) {
            ...
            const contract = await hardhat_1.ethers.getContractAt("Ownable", contractAddress);
            const callData = contract.interface.encodeFunctionData("transferOwnership", [EMERGENCY_CONFIG.EMERGENCY_ADDRESS]);
