...
async function main() {
    ...
    try {
        ...
        const paymentProcessor = await upgrades.deployProxy(
            ...
            [mockUSDCAddress, deployer.address], // _usdcToken, _owner
            { 
                kind: 'uups',
    ...
    try {
        ...
        const crawlLedger = await upgrades.deployProxy(
    ...
    try {
        const paymentProcessor = await ethers.getContractAt("PaymentProcessorUpgradeable", deployments.PaymentProcessor);
        const crawlLedger = await ethers.getContractAt("ProofOfCrawlLedgerUpgradeable", deployments.ProofOfCrawlLedger);
