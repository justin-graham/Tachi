...
async function main() {
  ...
  log(`\n🧪 Testing Upgradeable Contracts on ${network.name}...`);
  ...
  try {
    ...
    const crawlNFT = await ethers.getContractAt("CrawlNFTUpgradeable", crawlNFTAddress) as any;
    ...
    const paymentProcessor = await ethers.getContractAt("PaymentProcessorUpgradeable", paymentProcessorAddress) as any;
    ...
    const CrawlNFTV2 = await ethers.getContractFactory("CrawlNFTUpgradeable");
    await upgrades.validateUpgrade(crawlNFTAddress, CrawlNFTV2);
    log("✅ Upgrade validation passed");

    // Perform upgrade
    log("⬆️ Performing upgrade...");
    const crawlNFTUpgradedProxy = await upgrades.upgradeProxy(crawlNFTAddress, CrawlNFTV2);
    await crawlNFTUpgradedProxy.waitForDeployment();
    const crawlNFTUpgraded = await ethers.getContractAt("CrawlNFTUpgradeable", crawlNFTAddress) as any;
    ...
    try {
      const CrawlNFTUnauthorized = await ethers.getContractFactory("CrawlNFTUpgradeable", user1);
      await upgrades.upgradeProxy(crawlNFTAddress, CrawlNFTUnauthorized);
...
export { main as testUpgradeableContracts };
