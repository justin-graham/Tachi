...
contract CrawlNFT is ERC721, Ownable {
    ...
    event BatchLicenseMinted(uint256 indexed startTokenId, uint256 count);
    ...
    function batchMintLicenses(address[] calldata publishers, string[] calldata termsURIs) external onlyOwner {
    ...
    function getLicenseData(uint256 tokenId)
        external
        view
        returns (address publisher, bool isActive, uint32 mintTimestamp, uint32 lastUpdated)
    {
        if (!_exists(tokenId)) revert TokenNotFound();

        LicenseData memory data = _licenseData[tokenId];
        return (data.publisher, data.isActive, data.mintTimestamp, data.lastUpdated);
    }
    ...
    /// @notice Override approve to maintain ERC721 compatibility while keeping transfers blocked
    function approve(address to, uint256 tokenId) public override {
        super.approve(to, tokenId);
    }

    /// @notice Override setApprovalForAll for compatibility with tooling despite non-transferability
