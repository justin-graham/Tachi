...
contract ProofOfCrawlLedger is Ownable {
    ...
    function logCrawl(uint256 crawlTokenId, address crawler) external onlyOwner whenNotPaused {
        ...
        emit CrawlLogged(crawlTokenId, crawler, block.timestamp, logId);
    }
    ...
    function logCrawlWithContent(uint256 crawlTokenId, address crawler, bytes32 contentHash)
        external
        onlyOwner
        whenNotPaused
    {
        ...
        emit CrawlLoggedWithContent(crawlTokenId, crawler, contentHash, block.timestamp, logId);
    }
    ...
    function logCrawlWithURL(uint256 crawlTokenId, address crawler, string calldata url)
        external
        onlyOwner
        whenNotPaused
    {
        ...
        emit CrawlLoggedWithURL(crawlTokenId, crawler, url, block.timestamp, logId);
    }
    ...
    function logCrawlBatch(uint256[] calldata crawlTokenIds, address[] calldata crawlers)
        external
        onlyOwner
        whenNotPaused
    {
        ...
        for (uint256 i = 0; i < batchSize; i++) {
            ...
            emit CrawlLogged(crawlTokenIds[i], crawlers[i], block.timestamp, logId);
        }
