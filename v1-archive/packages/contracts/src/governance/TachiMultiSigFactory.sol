...
contract TachiMultiSigFactory is Ownable {
    ...
    event MultiSigDeployed(
        address indexed multiSig, address[] signers, uint256 requiredSignatures, bytes32 indexed salt
    );

    /// @notice Event emitted when a multi-sig wallet is upgraded
    event MultiSigUpgraded(address indexed oldMultiSig, address indexed newMultiSig, address[] signers);
    ...
    function deployProductionMultiSig(address[] calldata signers, bytes32 salt)
        external
        onlyOwner
    ...
    /// @param salt Salt for CREATE2 deployment
    /// @return multiSig Address of the deployed multi-sig wallet
    function deployTestnetMultiSig(address[] calldata signers, bytes32 salt)
        external
        onlyOwner
        returns (address multiSig)
    {
    ...
    /// @param salt Salt for CREATE2 deployment
    /// @return multiSig Address of the deployed multi-sig wallet
    function deployCustomMultiSig(address[] calldata signers, uint256 requiredSignatures, bytes32 salt)
        external
        onlyOwner
        returns (address multiSig)
    {
    ...
    function predictMultiSigAddress(address[] calldata signers, uint256 requiredSignatures, bytes32 salt)
        external
        view
        returns (address predicted)
    {
        bytes32 bytecodeHash =
            keccak256(abi.encodePacked(type(TachiMultiSig).creationCode, abi.encode(signers, requiredSignatures)));
    ...
    /// @param salt Salt for CREATE2 deployment
    /// @return multiSig Address of the deployed multi-sig wallet
    function _deployMultiSig(address[] calldata signers, uint256 requiredSignatures, bytes32 salt)
        internal
        returns (address multiSig)
    {
        // Validate signers
        ...
        bytes memory bytecode =
            abi.encodePacked(type(TachiMultiSig).creationCode, abi.encode(signers, requiredSignatures));
