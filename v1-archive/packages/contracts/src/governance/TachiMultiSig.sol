...
contract TachiMultiSig is ReentrancyGuard, Pausable {
     ...
     */
    function submitTransaction(address to, uint256 value, bytes calldata data, bool isEmergency)
        external
        onlyOwner
        returns (uint256)
    {
     ...
     */
    function confirmTransaction(uint256 txId) external onlyOwner txExists(txId) txNotExecuted(txId) {
        _confirmTransaction(txId, msg.sender);
    }
    ...
    function _confirmTransaction(uint256 txId, address owner) internal {
        ...
        // Auto-execute if threshold is reached and timelock has passed
        if (confirmationCount >= required && !transactions[txId].executed) {
            ...
            if (txn.isEmergency || block.timestamp >= txn.confirmationDeadline) {
                ...
                emit TransactionExecuted(txId, owner, success);
     ...
     * @param txId Transaction ID
     */
    function revokeConfirmation(uint256 txId) external onlyOwner txExists(txId) txNotExecuted(txId) {
     ...
     * @param txId Transaction ID
     */
    function executeTransaction(uint256 txId) external onlyOwner txExists(txId) txNotExecuted(txId) nonReentrant {
        Transaction storage txn = transactions[txId];
    ...
    function removeOwner(address ownerToRemove) external {
        ...
        // Mark the owner as inactive before mutating the array to keep isOwner checks consistent mid-call
        ownerInfo[ownerToRemove].isActive = false;
