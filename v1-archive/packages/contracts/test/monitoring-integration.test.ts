import { TachiSecurityMonitor } from "../src/monitoring/TachiSecurityMonitor";
...
describe("Tachi Production Monitoring - Integration Tests", () => {
  let securityMonitor: TachiSecurityMonitor;
  ...
  beforeEach(() => {
    ...
    securityMonitor = new TachiSecurityMonitor(mockConfig);
    performanceMonitor = new PerformanceMonitor(securityMonitor);
  ...
  describe("Real-time Security Monitoring", () => {
    it("should initialize security monitor successfully", () => {
      expect(securityMonitor).to.not.be.undefined;
      const stats = securityMonitor.getStats() as MonitorStats;
      expect(stats).to.have.property("isMonitoring");
      expect(stats).to.have.property("alertsGenerated");
      expect(stats).to.have.property("metricsBuffered");
    });

    it("should provide monitoring statistics", () => {
      const stats = securityMonitor.getStats() as MonitorStats;
      expect(stats.isMonitoring).to.be.a('boolean');
      expect(stats.alertsGenerated).to.be.a('number');
      expect(stats.metricsBuffered).to.be.a('number');
      expect(stats.uptime).to.be.a('number');
      expect(stats.memoryUsage).to.be.an('object');
    });
  ...
  describe("Performance Monitoring", () => {
    ...
    it("should generate dashboard data", () => {
      ...
      expect(dashboardData).to.have.property("systemHealth");
      expect(dashboardData).to.have.property("networkMetrics");
      expect(dashboardData).to.have.property("contractMetrics");
      expect(dashboardData).to.have.property("alertsSummary");
      expect(dashboardData).to.have.property("performanceMetrics");
      
      expect(dashboardData.systemHealth).to.have.property("status");
      expect(dashboardData.systemHealth).to.have.property("uptime");
      expect(dashboardData.systemHealth).to.have.property("memoryUsage");
    });

    it("should provide system health status", () => {
      const dashboardData = performanceMonitor.getDashboardData();
      expect(dashboardData.systemHealth.status).to.be.a('string');
      expect(['healthy', 'warning', 'critical']).to.include(dashboardData.systemHealth.status);
    });
  });
  ...
  describe("Error Tracking and Alerting", () => {
    it("should track metrics buffer size", () => {
      const stats = securityMonitor.getStats() as MonitorStats;
      expect(stats.metricsBuffered).to.be.a('number');
      expect(stats.metricsBuffered).to.be.at.least(0);
    });

    it("should initialize alert tracking", () => {
      const stats = securityMonitor.getStats() as MonitorStats;
      expect(stats.alertsGenerated).to.be.a('number');
      expect(stats.alertsGenerated).to.be.at.least(0);
    });
  ...
  describe("System Integration", () => {
    ...
    it("should maintain state consistency", () => {
      const stats1 = securityMonitor.getStats() as MonitorStats;
      const stats2 = securityMonitor.getStats() as MonitorStats;
      
      expect(stats1.uptime).to.be.at.most(stats2.uptime);
      expect(stats1.memoryUsage).to.be.an('object');
      expect(stats2.memoryUsage).to.be.an('object');
    });
