...
-- Backup Log table - Track all backup operations
...
-- Restore Log table - Track all restore operations
...
-- Database Performance Metrics table - Store performance data over time
...
-- Slow Query Log table - Track slow database queries for optimization
...
-- Database Maintenance Log table - Track maintenance operations
...
-- Connection Audit Log table - Track database connection patterns
...
-- Query Performance Cache table - Cache query performance data
...
CREATE INDEX idx_backup_log_backup_id ON backup_log(backup_id);
CREATE INDEX idx_backup_log_type_status ON backup_log(type, status);
CREATE INDEX idx_backup_log_start_time ON backup_log(start_time);

CREATE INDEX idx_restore_log_backup_id ON restore_log(backup_id);
CREATE INDEX idx_restore_log_restore_time ON restore_log(restore_time);

CREATE INDEX idx_db_performance_metrics_time ON db_performance_metrics(metric_time);
CREATE INDEX idx_db_performance_metrics_time_desc ON db_performance_metrics(metric_time DESC);
...
CREATE INDEX idx_slow_query_log_priority ON slow_query_log(priority);
CREATE INDEX idx_slow_query_log_last_seen ON slow_query_log(last_seen DESC);

CREATE INDEX idx_db_maintenance_log_operation ON db_maintenance_log(operation_type);
CREATE INDEX idx_db_maintenance_log_table ON db_maintenance_log(table_name);
CREATE INDEX idx_db_maintenance_log_start_time ON db_maintenance_log(start_time);

CREATE INDEX idx_connection_audit_event_type ON connection_audit_log(event_type);
CREATE INDEX idx_connection_audit_user ON connection_audit_log(user_type, user_id);
CREATE INDEX idx_connection_audit_created_at ON connection_audit_log(created_at);

CREATE INDEX idx_query_performance_pattern ON query_performance_cache(query_pattern);
CREATE INDEX idx_query_performance_expires ON query_performance_cache(cache_expires);
CREATE INDEX idx_query_performance_score ON query_performance_cache(performance_score DESC);

-- Full-text search index for slow queries
CREATE INDEX idx_slow_query_text_fts ON slow_query_log USING GIN(to_tsvector('english', query_text));

-- Partial indexes for active/recent records
CREATE INDEX idx_slow_query_recent ON slow_query_log(last_seen) WHERE last_seen > NOW() - INTERVAL '7 days';
CREATE INDEX idx_db_performance_recent ON db_performance_metrics(metric_time) WHERE metric_time > NOW() - INTERVAL '24 hours';
...
BEGIN
    ...
    FROM (
        SELECT 
            ...
            (SELECT COUNT(*) FROM information_schema.tables WHERE table_name = analyze_table_performance.table_name) as row_count,
            pg_total_relation_size(analyze_table_performance.table_name::regclass) as total_size,
            pg_relation_size(analyze_table_performance.table_name::regclass) as table_size,
            pg_total_relation_size(analyze_table_performance.table_name::regclass) - pg_relation_size(analyze_table_performance.table_name::regclass) as index_size
    ) t
    LEFT JOIN pg_stat_user_tables s ON s.relname = analyze_table_performance.table_name;
END;
$$ LANGUAGE plpgsql;
...
('backup_system', 'healthy', NOW()),
('performance_monitoring', 'healthy', NOW()),
('connection_pooling', 'healthy', NOW());
