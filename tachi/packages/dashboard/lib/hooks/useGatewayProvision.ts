...
import { showApiSuccess, showApiError, showApiLoading, showSuccess, showError } from '../toast'
...
export function useGatewayProvision() {
  ...
  const { 
    ...
    queryFn: async (): Promise<ProvisionedGateway[]> => {
      ...
      const response = await fetch(`/api/gateway/list/${address}`)
  ...
  const provisionGateway = useMutation({
    mutationFn: async (params: ProvisionGatewayParams): Promise<ProvisionedGateway> => {
      ...
      const response = await fetch('/api/gateway/provision', {
        method: 'POST',
        headers: {
