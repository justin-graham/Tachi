...
exports.deployProductionMultiSig = main;
 ...
 */
async function verifyHardwareWalletSigner(signer) {
    try {
        console.log(`🔐 Verifying hardware wallet signer: ${signer.address} (${signer.role})`);
        // Check if address is valid
        if (!hardhat_1.default.ethers.isAddress(signer.address)) {
        ...
        const balance = await hardhat_1.default.ethers.provider.getBalance(signer.address);
        console.log(`   💰 Balance: ${hardhat_1.default.ethers.formatEther(balance)} ETH`);
        // Check if it's likely a hardware wallet (has some ETH but not too much)
        const balanceETH = parseFloat(hardhat_1.default.ethers.formatEther(balance));
        if (balanceETH < 0.01) {
 ...
 */
async function preDeploymentChecklist(config) {
 ...
 */
async function deployProductionMultiSig(config) {
    console.log("\n🚀 Deploying Production Multi-Signature Wallet...");
    const [deployer] = await hardhat_1.default.ethers.getSigners();
    console.log(`📝 Deployer: ${deployer.address}`);
    console.log(`💰 Deployer balance: ${hardhat_1.default.ethers.formatEther(await deployer.provider.getBalance(deployer.address))} ETH`);
    ...
    const TachiProductionMultiSig = await hardhat_1.default.ethers.getContractFactory("TachiProductionMultiSig");
    const multiSig = await TachiProductionMultiSig.deploy(signerAddresses, signerRoles, hardwareWalletTypes, config.requiredSignatures, config.timeLockDuration);
 ...
 */
async function transferContractOwnership(multiSigAddress) {
    ...
    for (const contractInfo of contractsToTransfer) {
        try {
            ...
            const contract = await hardhat_1.default.ethers.getContractAt(contractInfo.contractName, contractInfo.address);
            // Check current owner
 ...
 */
async function verifyDeployment(multiSigAddress, config) {
    console.log("\n🔍 Verifying Deployment...");
    try {
        const multiSig = await hardhat_1.default.ethers.getContractAt("TachiProductionMultiSig", multiSigAddress);
        // Verify owners
 ...
 */
function generateDeploymentReport(deploymentResult, config, migratedContracts) {
    const network = hardhat_1.default.network.name;
    const chainId = Number(hardhat_1.default.network.config.chainId);
    const report = {
        network,
        chainId,
 ...
 */
function saveDeploymentArtifacts(report) {
    const deploymentsDir = (0, path_1.join)(__dirname, "..", "deployments", "production");
    if (!(0, fs_1.existsSync)(deploymentsDir)) {
        (0, fs_1.mkdirSync)(deploymentsDir, { recursive: true });
    }
    const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
    const reportFile = (0, path_1.join)(deploymentsDir, `multisig-deployment-${timestamp}.json`);
    ...
    const latestFile = (0, path_1.join)(deploymentsDir, "latest.json");
    (0, fs_1.writeFileSync)(latestFile, JSON.stringify(report, null, 2));
    console.log(`📄 Latest deployment: ${latestFile}`);
}
/**
 * Post-deployment instructions
 */
function printPostDeploymentInstructions(report) {
    console.log("\n" + "=".repeat(60));
