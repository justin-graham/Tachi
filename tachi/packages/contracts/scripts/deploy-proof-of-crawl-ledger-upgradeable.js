...
async function main() {
    ...
    const proxy = await hardhat_1.upgrades.deployProxy(ProofOfCrawlLedgerUpgradeable, [deployer.address], // initializer arguments: _owner
    {
        kind: 'uups',
    ...
    try {
        ...
        const crawlExists = await contract.hasCrawlBeenLogged(1, deployer.address, (await hardhat_1.ethers.provider.getBlock('latest')).timestamp);
